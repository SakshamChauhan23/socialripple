...
@RequestMapping("/v1/api/linkedin")
public class LinkedInConnectionController {
	...
	public void callback(@RequestParam("code") String oauthVerifier,
			@RequestParam("state" ) String transactionId,
		...
		try {
			...
			UserAuthToken savedToken = xConnectionService.handleLinkedInCallback(oauthVerifier, token, session, transactionId);
			...
			try {
				redirectWithStatus(response, token, "error", "token_exchange_failed");
			} catch (Exception ex) {
