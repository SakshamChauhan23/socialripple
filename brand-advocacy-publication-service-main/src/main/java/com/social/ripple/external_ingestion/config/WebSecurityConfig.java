...
import com.social.ripple.external_ingestion.security.jwt.AuthTokenFilter;
import com.social.ripple.external_ingestion.security.jwt.JwtUtils;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, AuthTokenFilter authTokenFilter) {
        try {
            ...
            http.cors(cors -> cors.configurationSource(corsConfigurationSource())).csrf(csrf -> csrf.disable())
                    .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)).exceptionHandling(ex -> ex.authenticationEntryPoint(authenticationEntryPoint()).accessDeniedHandler(accessDeniedHandler())).authorizeHttpRequests(auth ->
                            auth.requestMatchers("/v1/api/x/auth",
                                            "/v1/api/x/callback",
                                            "/v1/api/linkedin/callback",
                                            "/v1/api/fb/callback",
                                            "/v1/api/instagram/callback",
                                            "/error",
                                            "/v1/api/org/business-pages/facebook/callback",
                                            "/v1/api/org/business-pages/instagram/callback",
                                            "/v1/api/org/business-pages/linkedin/callback",
                                            "/v1/api/org/business-pages/x/callback",
                                            "/v1/api/x/fetch/x",
                                            "/v1/api/x/fetch/linkedin",
                                            "/v1/api/x/fetch/linkedin-v",
                                            "/v1/api/x/fetch/facebook",
                                            "/v1/api/x/fetch/instagram",
                                            "/v1/api/x/fetch/x-leader"
//                    "/v1/api/x/tweet",
//                                    "/v1/api/posts/**"
                                    )
                                    .permitAll()
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        try {
            ...
            corsConfiguration.setAllowedHeaders(Arrays.asList("Authorization", "Content-Type", "Accept", "X-Requested-With", "X-Trace-Id", "X-Correlation-Id", "X-Tenant-Id", "Language-Id", "Accept-Language"));
