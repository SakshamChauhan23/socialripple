 ...
 */
package com.social.ripple.external_ingestion.security.jwt;
...
public class AuthTokenFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			log.info("[{}]|AUTH|FILTER|Request path={}", traceId, request.getRequestURI());
			...
			log.info("[{}]|JWT|PARSE|Extracted subject={}", traceId, userId);
	...
	private String parseJwt(HttpServletRequest request, String traceId) {
		...
		if (StringUtils.hasText(headerAuth) && headerAuth.startsWith("Bearer ")) {
			log.info("[{}]|JWT|PARSE|Authorization header found for path={}", traceId, request.getRequestURI());
			return headerAuth.substring(7);
		}

		log.warn("[{}]|JWT|PARSE|Authorization header is missing or malformed for path={}", traceId, request.getRequestURI());
		return null;
	}
}
