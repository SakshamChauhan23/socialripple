/**
 * Filename: PostHashtagRepository.java © Copyright 2024 Quasarix. ALL RIGHTS RESERVED. All rights, title and interest (including all intellectual
...
import com.social.ripple.external_ingestion.dao.model.PostHashtag;
...
public interface PostHashtagRepository extends JpaRepository<PostHashtag, Long> {
	@Query("SELECT h FROM PostHashtag h WHERE h.post.id IN :postIds")
	List<PostHashtag> findByPostIds(@Param("postIds")
	List<Long> postIds);

	@Query("SELECT h.hashTag FROM PostHashtag h WHERE h.post.id = :postId")
	List<String> fetchByPostId(@Param("postId") Long postId);
