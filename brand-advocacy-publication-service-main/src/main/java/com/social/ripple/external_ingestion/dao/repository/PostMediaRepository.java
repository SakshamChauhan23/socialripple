/**
 * Filename: PostMediaRepository.java © Copyright 2024 Quasarix. ALL RIGHTS RESERVED. All rights, title and interest (including all intellectual
...
public interface PostMediaRepository extends JpaRepository<PostMedia, Long> {
	...
	@Query(value = "SELECT file_url FROM post_media WHERE post_id = :postId", nativeQuery = true)
	List<String> findUrlsByPostId(@Param("postId")
	Long postId);
		 
	@Query("SELECT m FROM PostMedia m WHERE m.postId IN :postIds")
	List<PostMedia> findByPostIds(@Param("postIds") List<Long> postIds);

	@Query("SELECT m FROM PostMedia m WHERE m.postId = :postId")
