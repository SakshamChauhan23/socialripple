...
@RequestMapping("/v1/api/fb")
public class FbConnectionController {
	...
	public void callback(@RequestParam("code") String oauthVerifier,
		...
		try {
			...
			UserAuthToken savedToken = facebookConnectionService.handleFbCallback(oauthVerifier, token, session, transactionId);
		...
		while (current != null) {
			if (current instanceof ResponseStatusException responseStatusException) {
				...
				if (reason != null) {
					...
					if (reason.contains("No Facebook pages found")) {
						return "no_facebook_pages_found";
					}
	...
	public ResponseEntity<?> createPost(@RequestHeader(value = "x-trace-id", required = true) String xTraceId,
		...
		try {
			// Pre-validate: check if user has a connected Facebook account
			if (userDetails == null) {
				TweetResponse err = new TweetResponse();
				err.setStatus(false);
				err.setMessage("Authentication required to publish.");
				return ResponseEntity.status(401).body(err);
			}

			TweetResponse response = facebookConnectionService.createFbPost(request, userDetails);
			return ResponseEntity.ok(response);
		} catch (Exception e) {
			log.error("Error | fb-post | traceId={} | message={}", xTraceId, e.getMessage(), e);
			TweetResponse err = new TweetResponse();
			err.setStatus(false);
			err.setMessage("Failed to publish: " + e.getMessage());
