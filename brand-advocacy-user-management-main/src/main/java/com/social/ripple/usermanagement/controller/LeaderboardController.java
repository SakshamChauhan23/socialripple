...
import com.social.ripple.usermanagement.dao.model.Leaderboard;
import com.social.ripple.usermanagement.dao.model.Post;
import com.social.ripple.usermanagement.dao.repository.LeaderboardRepository;
...
@RestController
@RequestMapping("{version}/leaderboard")
public class LeaderboardController {
	@Autowired private LeaderboardRepository leaderboardRepository;

	@GetMapping("top")
	public ResponseEntity<BaseResponse> fetchLeaders(@PathVariable("version")
	...
	String tenantId) {
		try {
			...
			Page<Leaderboard> postPage = leaderboardRepository.findByTenantIdOrderByTotalPointsDesc(userDetails.getOrganization().getId(),pageable);

			List<Leaderboard> posts = postPage.getContent();

			List<LeaderDTO> leaderDTOs = new ArrayList<>();

			posts.forEach(x->{
				leaderDTOs.add(new LeaderDTO(x.getUser().getName(),x.getTotalPoints()));
			});


			LeaderResponse response = new LeaderResponse();
			response.setLeaderList(leaderDTOs);
