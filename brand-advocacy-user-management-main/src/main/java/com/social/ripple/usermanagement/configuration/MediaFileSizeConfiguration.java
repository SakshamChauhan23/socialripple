/**
 * Filename: MediaFileSizeConfiguration.java © Copyright 2024 Quasarix. ALL RIGHTS RESERVED. All rights, title and interest (including all
...
public class MediaFileSizeConfiguration {
	...
	public MultipartConfigElement multipartConfigElement() {
		...
		try {
			log.info("Configuring multipart file size limits");
			MultipartConfigFactory factory = new MultipartConfigFactory();

			factory.setMaxFileSize(DataSize.ofMegabytes(Integer.valueOf(AppCache.configParameters.get(ConfigKeys.MAX_FILE_SIZE).getConfigValue())));

			factory.setMaxRequestSize(
					DataSize.ofMegabytes(Integer.valueOf(AppCache.configParameters.get(ConfigKeys.MAX_REQUEST_SIZE).getConfigValue())));

			factory.setFileSizeThreshold(
					DataSize.ofMegabytes(Integer.valueOf(AppCache.configParameters.get(ConfigKeys.FILE_SIZE_THRESHOLD).getConfigValue())));
			log.info("Multipart file size limits configured successfully");
			return factory.createMultipartConfig();
		}
		catch (NumberFormatException e) {
			log.error("Invalid configuration value for file sizes", e);
			throw new IllegalArgumentException("Invalid configuration value for file sizes", e);
		}
		catch (Exception e) {
			log.error("Error configuring multipart file size", e);
			throw new RuntimeException("Error configuring multipart file size", e);
		}
