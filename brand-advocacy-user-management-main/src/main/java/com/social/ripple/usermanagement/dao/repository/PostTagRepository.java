/**
 * Filename: PostTagRepository.java © Copyright 2024 Quasarix. ALL RIGHTS RESERVED. All rights, title and interest (including all intellectual
...
public interface PostTagRepository extends JpaRepository<PostTag, Long> {
	...
	@Query("SELECT t FROM PostTag t WHERE t.postId IN :postIds")
	List<PostTag> findByPostIds(@Param("postIds") List<Long> postIds);

	@Query("SELECT t.postId FROM PostTag t WHERE t.taggedUserId = :userId")
	List<Long> findPostIdsByTaggedUserId(@Param("userId") Long userId);
