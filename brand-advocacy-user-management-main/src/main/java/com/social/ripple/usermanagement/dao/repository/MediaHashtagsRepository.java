/**
 * Filename: MediaHashtagsRepository.java © Copyright 2024 Quasarix. ALL RIGHTS RESERVED. All rights, title and interest (including all intellectual
...
public interface MediaHashtagsRepository extends JpaRepository<MediaHashtag, Long> {

	@Query("SELECT h.hashtag FROM MediaHashtag h WHERE h.libraryId = :libraryId")
	List<String> findHashtagsByLibraryId(@Param("libraryId")
	Long libraryId);

	void deleteByLibraryId(Long libraryId);

	MediaHashtag findByLibraryIdAndHashtag(Long libraryId, String trimmedTag);

	List<MediaHashtag> findByLibraryId(Long libraryId);
