 ...
 */
package com.social.ripple.usermanagement.security.service;
...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String inputUserName) throws UsernameNotFoundException {
		...
		try {
			...
			log.debug("[{}]|CONFIGURATION|UserFound|User ID: {}, Email: {}", traceId, user.getId(), user.getEmail());

			List<String> roles = userRoleRepository.findRoleNamesByUserId(user.getId());
			log.debug("[{}]|CONFIGURATION|RoleFetch|Roles fetched: {}", traceId, roles);
